...
//
// CANONICAL COPY — lives in supabase/functions/_shared/ so both the SvelteKit
// app (via the src/lib/server/libcal.js re-export shim) and the watchlist
// worker edge function share it. Keep this file runtime-neutral: fetch only,
// no $env / node / Deno APIs. Callers may pass an alternate base URL (test
// stubs); it defaults to the real LibCal host.

export const LIBCAL_BASE = 'https://northeastern.libcal.com';
...
const spacesPage = (base) => `${base}/reserve/spaces/studyspace`;
 ...
 */
export function parseRooms(html, base = LIBCAL_BASE) {
  const rooms = [];
  ...
  while ((match = blockRe.exec(html)) !== null) {
    ...
    rooms.push({
      ...
      bookUrl: url ? `${base}${decodeJsString(url[1])}` : spacesPage(base),
    });
  }
  return rooms;
}

export async function fetchRooms(base = LIBCAL_BASE) {
  const res = await fetch(spacesPage(base), { headers: COMMON_HEADERS });
  if (!res.ok) {
    throw new Error(`LibCal spaces page returned ${res.status}`);
  }
  const html = await res.text();
  const rooms = parseRooms(html, base);
  if (rooms.length === 0) {
 ...
 */
export async function fetchGrid(start, end, base = LIBCAL_BASE) {
